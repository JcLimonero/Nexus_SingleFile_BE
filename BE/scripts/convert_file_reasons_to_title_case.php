<?php
/**
 * Convertir motivos de file_reasons de MAYÚSCULAS a Title Case
 */

error_reporting(E_ALL);
ini_set('display_errors', 1);

echo "=== CONVERTIR FILE_REASONS A TITLE CASE ===\n\n";
...
function toTitleCase($text) {
    ...
    $acronyms = ['ID', 'RFC', 'CURP', 'CFDI', 'PDI', 'VGD', 'REPUVE', 'AISE', 'PROFECO', 'KIA', 'IF', 'DCTO'];
...
try {
    ...
    echo "📋 Obteniendo registros de 'file_reasons'...\n";
    $records = $mysqli->query("SELECT Id, Name FROM file_reasons ORDER BY Id");
    ...
    if ($totalRecords == 0) {
        echo "⚠️  No hay registros para actualizar\n";
        $mysqli->close();
        exit(0);
    }
    
    echo "🔄 Convirtiendo a Title Case...\n";
    echo str_repeat("=", 80) . "\n";
    ...
    while ($row = $records->fetch_assoc()) {
        ...
        $updateQuery = $mysqli->prepare("UPDATE file_reasons SET Name = ?, UpdateDate = NOW() WHERE Id = ?");
    ...
    // Mostrar todos los motivos actualizados
    echo "📋 MOTIVOS ACTUALIZADOS:\n";
    echo str_repeat("=", 80) . "\n";
    $allReasons = $mysqli->query("SELECT Id, Name FROM file_reasons ORDER BY Id");
