 ...
 * Versión automática (sin confirmación interactiva)
 * 
 * Uso: php execute_038_convert_columns_auto.php
 */
...
try {
    ...
    foreach ($statements as $index => $statement) {
        ...
        if (preg_match('/ALTER TABLE `([^`]+)` CHANGE COLUMN `([^`]+)` `([^`]+)`/', $statement, $matches)) {
            ...
            $checkOldQuery = "SELECT COUNT(*) as count FROM INFORMATION_SCHEMA.COLUMNS 
                             WHERE TABLE_SCHEMA = DATABASE() 
                             AND TABLE_NAME = '$table' 
                             AND COLUMN_NAME = '$oldColumn'";
            
            $checkOldResult = $mysqli->query($checkOldQuery);
            if ($checkOldResult) {
                $rowOld = $checkOldResult->fetch_assoc();
                if ($rowOld['count'] == 0) {
                    echo "⚠️  $table.$oldColumn (no existe, saltando)\n";
                    $skippedCount++;
                    continue;
                }
            }
