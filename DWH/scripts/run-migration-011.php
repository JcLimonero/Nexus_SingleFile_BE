...
<?php
/**
 * Ejecuta la migración 011 (bussines_name, tipo_cliente, tipo_operacion, tipo_proceso)
 */
...
$cols = ['bussines_name' => 'VARCHAR(200)', 'tipo_cliente' => 'VARCHAR(20)', 'tipo_operacion' => 'VARCHAR(50)', 'tipo_proceso' => 'VARCHAR(50)'];
...
// Backfill bussines_name y tipo_cliente desde nexfile_customers
$connCol = null;
$ndCol = null;
$bussinesCol = null;
$tipoCol = null;
...
foreach (['bussines_name', 'razon_social', 'razonSocial'] as $c) {
    if (in_array($c, $custCols, true)) { $bussinesCol = $c; break; }
}
foreach (['tipo_cliente'] as $c) {
...
if ($connCol && $ndCol && $bussinesCol && $invConnCol && $invCustCol && $invIdAgencyCol) {
    ...
    $setTipo = $tipoCol ? "i.tipo_cliente = COALESCE(NULLIF(TRIM(c.`$tipoCol`), ''), 'fisica')" : "i.tipo_cliente = 'fisica'";
    ...
    echo "Backfill bussines_name/tipo_cliente: $affected registros actualizados.\n";
}
